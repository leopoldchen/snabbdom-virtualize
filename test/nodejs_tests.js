...
const {
    vnode: VNode
} = require('snabbdom/vnode');


const opts = {
    context: (typeof document != 'undefined') ? document : jsdom('<html></html>')
};
...
describe("In a nodejs environment", () => {
    describe("virtualizeString", () => {
        ...
        it("should decode string with classes", () => {
            expect(virtualizeString('<span class="foo" style="background-color: blue; padding-left: 5px;" dir="rtl" data-test-attr="test" />', opts))
                .to.deep.equal(
                    h('span', {
                        class: {
                            foo: true
        ...
        it("should handle comments in HTML strings", () => {
            expect(
                virtualizeString('<div> <!-- First comment --> <span>Hi</span> <!-- Another comment --> Something</div>', opts)
            ).to.deep.equal(
                h('div', [
                    ' ',
                    ' ',
